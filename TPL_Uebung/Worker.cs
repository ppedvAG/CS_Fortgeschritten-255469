...
public class Worker
{
	...
	public void DoWork()
	{
		while (true)
		{
			if (Program.ImagePaths.IsEmpty)
				continue;

			bool gotFilePath = Program.ImagePaths.TryDequeue(out string filePath);
			if (gotFilePath)
			{
				string fileName = Path.GetFileName(filePath);
				string outputPath = Path.Combine(Program.OutputPath, fileName);
				Program.ProcessedImages.Add(filePath); //Quelldatei hinzufügen, damit der Scanner diese nicht mehr einliest
